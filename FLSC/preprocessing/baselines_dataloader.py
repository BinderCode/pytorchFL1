...
def divide_data(num_client=1, num_local_class=10, dataset_name='emnist', i_seed=0):
    ...
    trainset_config = {'users': [],
                       'user_data': {},
                       'num_samples': []}
    config_division = {}  # Count of the classes for division   数据集划分的数量
    config_class = {}  # Configuration of class distribution in clients  客户端类分布的配置
    config_data = {}  # Configuration of data indexes for each class : Config_data[cls] = [0, []] | pointer and indexes 每个类的数据索引配置
