...
class FedClient(object):
    def __init__(self, name, epoch, dataset_id, model_name):
        ...
        """
        # Initialize the metadata in the local client   初始化了客户端的一些局部变量
        ...
        # Initialize the local model   初始化了客户端的一些局部变量
    ...
    def load_trainset(self, trainset):
        """
        Client loads the training dataset. 加载训练数据集，并统计数据点的数量。
    ...
    def update(self, model_state_dict):  
        """
        Client updates the model from the server.   #用从服务器接收到的全局模型（model_state_dict）来更新客户端的本地模型。
    ...
    def train(self):
        """
        在本地数据集上训练模型。使用 SGD（随机梯度下降）优化器和交叉熵损失函数。
